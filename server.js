...
app.post("/which-gen", async (req, res) => {
  try {
    ...
    //Get the picture
    const searchPersonPicRoute = $(".infobox-image").find("a").attr("href");
    const searchPersonPicSrc = `https://wikipedia.org/${searchPersonPicRoute}`;
    searchPerson.src = searchPersonPicSrc;

    //Get the wikipedia name
    searchPerson.name = $(".infobox-above").text();

    //Get the birth date and the gen
